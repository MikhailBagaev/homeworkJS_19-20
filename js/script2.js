var parsedArray = JSON.stringify([
  {
...
var data = JSON.parse(parsedArray);

//skills

  var skills = [];
_(data).forEach(function(value) {
 skills.push(_.get( value ,'skills'));
});
skills = _.uniq(_.flattenDeep(skills, 2)).sort(function(a, b){
  var A = a.toLowerCase();
  var B = b.toLowerCase();
  if (A < B){
     return -1;
...
console.log('Unique skills', skills);

//people collected by amount of friends

var arr = [];
_(data).forEach(function(value) {
 arr.push( _.pick(value,['name','friends']));
});
arr = _.sortBy(arr,'friends');
var people = [];
(arr).forEach(function(value) {
    people.push(_.get(value,'name'));
})
people = _.reverse(people);

console.log('People collected by friends number', people);

//array of friends

var friends = [];
_(data).forEach(function(value) {
 _(_.get( value ,'friends' )).forEach(function (value) {
   friends.push(_.get( value ,'name' ))
 });
});
...
console.log('Unique list of friends',friends);